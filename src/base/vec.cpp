...
mat33 inverse(const mat33& m) {
	vec3 inv_d(1.0f / (m.x.x * (m.y.y * m.z.z - m.z.y * m.y.z) - m.y.x * (m.x.y * m.z.z - m.z.y * m.x.z) + m.z.x * (m.x.y * m.y.z - m.y.y * m.x.z)));

	return mat33(
			vec3(+(m.y.y * m.z.z - m.z.y * m.y.z), -(m.y.x * m.z.z - m.z.x * m.y.z), +(m.y.x * m.z.y - m.z.x * m.y.y)) * inv_d,
			vec3(-(m.x.y * m.z.z - m.z.y * m.x.z), +(m.x.x * m.z.z - m.z.x * m.x.z), -(m.x.x * m.z.y - m.z.x * m.x.y)) * inv_d,
			vec3(+(m.x.y * m.y.z - m.y.y * m.x.z), -(m.x.x * m.y.z - m.y.x * m.x.z), +(m.x.x * m.y.y - m.y.x * m.x.y)) * inv_d
		);
}
