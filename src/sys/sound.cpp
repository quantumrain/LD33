...
#include "base.h"
#include "sound.h"
#include "render.h" // todo: remove when asset loading is moved out
...
void init_sound() {
	...
	while(wav_header* wav = (wav_header*)find_next_asset(&fad, ASSET_WAV, &name)) {
		...
		if (sound_fx* fx = acquire_sound_fx(prefix)) {
			sound_id id = audio_create_sound(wav);

			if (id)
				fx->variant.push_back(id);
		}
	}
}
...
void finalise_sound() {
	for(auto& fx : g_sound_fx) {
		...
		int format = audio_format(fx.variant.front());

		if (!fx.max_voices) {
			debug("finalise_sound: no sound mapping set for %s", fx.name);
		}

		for(int i = 0; i < fx.max_voices; i++)
			fx.voice.push_back(audio_create_voice(format));
	}
}
...
voice_id sound_play(sfx s, float semitones, float decibels, int flags) {
	...
	if (best) {
		sound_id sid = fx->variant[g_sound_rand.rand(fx->variant.size())];

		audio_play(best->id, sid, semitones, decibels, flags);
