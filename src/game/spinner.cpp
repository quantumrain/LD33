...
spinner::spinner() : entity(ET_SPINNER), _set_home(), _t() {
	set_radius(20.0f);
}
...
void spinner::tick(int move_clipped) {
	if (!_set_home) {
		_set_home = true;
		_home = _pos;
	}
	...
	vec2 target = _home + rotation(_t) * 160.0f;

	vec2 delta = target - _pos;
	...
	interact_enemy_and_player(this, rgba(1.2f, 0.2f, 1.2f, 1.0f));

	avoid_enemy(_world, this);
}

void spinner::draw(draw_context* dc) {
	dc->translate(_pos.x, _pos.y);
	dc->rotate_z(_rot);

	float f = 0.2f + fabsf(cosf(_rot * 3.75f));

	dc->shape(vec2(), 4, _radius * 0.75f, _rot, rgba(0.8f * f, 0.2f, 0.8f * f, 1.0f));
	dc->shape(vec2(), 4, _radius * 0.25f, _rot, rgba(0.8f * (1.0f - f), 0.2f, 0.8f * (1.0f - f), 1.0f));
